function appendLicensesToCase(executionContext) {
    ...
    if (!customer) {
        formContext.getAttribute("new_globalsearchlicense").setValue(null);
        return;
    }
    ...
    if (customerType === "account") {
        ...
        Xrm.WebApi.retrieveRecord("contact", customerId, "?$select=new_companyname").then(
            function(contact) {
                if (contact.new_companyname) {
                    ...
                    formContext.getAttribute("new_globalsearchlicense").setValue("Contact not linked to any company");
                }
            },
            function(error) {
        ...
        formContext.getAttribute("new_globalsearchlicense").setValue("Unsupported customer type");
    }
}
...
<condition attribute="new_companyname" operator="eq" value="${accountId}" />
    ...
    Xrm.WebApi.retrieveMultipleRecords("new_licensetype", "?fetchXml=" + encodeURIComponent(fetchXml)).then(
        function(result) {
            ...
            formContext.getAttribute("new_globalsearchlicense").setValue(licenseCsv);
        },
        function(error) {
