function moveToTicketClosureStage(executionContext) {
    try {
        ...
        const flagAttr = formContext.getAttribute("new_isticketclose");
        if (!flagAttr) {
            console.error("❌ Field 'new_isticketclose' not found.");
            return;
        }
        ...
        const bpfEntityName = "phonetocaseprocess"; // Your BPF entity logical name
        const targetStageName = "Ticket Closure";   // Target stage name
            ...
            const proceedToStageUpdate = () => {
                ...
                Xrm.WebApi.retrieveMultipleRecords("processstage", `?$filter=stagename eq '${targetStageName}'`).then(function (stageResult) {
                    ...
                    Xrm.WebApi.updateRecord(bpfEntityName, bpfId, updatePayload).then(function () {
                        console.log("✅ BPF moved to 'Ticket Closure' stage.");
